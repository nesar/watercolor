...
# %% ../nbs/03_calculate_csp.ipynb #a02b28f9
import numpy as np
...
# %% ../nbs/03_calculate_csp.ipynb #689fd44b
...
# %% ../nbs/03_calculate_csp.ipynb #835aaf73
def calc_fluxes_for_galaxy(gal_file_in:str=GALS_FILE, # HACC stellar catalog, 
    ...
    ## This should not be done -- this repeats the loading
    # _, _, gal_tags, _, metal_hydro, mass, age_hydro, _, _, _, _, _, _ = load_hacc_galaxy_data(gal_file_in)
    _, _, gal_tags, _, metal_hydro, mass, age_hydro, _, _, _, _, _, _ = load_hacc_galaxy_data_opencosmo(particle_file_in, gal_file_in) ## This should not be done -- this repeats the loading
...
# %% ../nbs/03_calculate_csp.ipynb #f9a08839
def calc_fluxes_for_individual_galaxy(galaxy_tags_hacc:np.array=None, # HACC galaxy tags,
                                      ...
                                      age_hacc:np.array=None, # Age in the HACC catalog,                                      
                                      unique_gal_tag:np.float32=None, # Selected galaxy tag
                                      spectral_library_flux:str=LIBRARY_FLUX_FILE, # Fluxes in the spectral library,
                                      spectral_library_wave:str=LIBRARY_WAVE_FILE, # Wavelengths in the spectral library,
                                      spectral_library_age:str=LIBRARY_AGE_FILE, # Age in the spectral library,
    ...
    gal_tag_cond = np.where(galaxy_tags_hacc == unique_gal_tag)
    # print(f"Selected galaxy tag: {unique_gal_tag}")
    # print('Galaxy tags HACC', galaxy_tags_hacc)
    # print(f"Number of star particles in the selected galaxy: {gal_tag_cond[0].shape[0]}")

    spec_flux_ssp = np.zeros( shape=(gal_tag_cond[0].shape[0], spec_wave_library.shape[0]))

    for idx, ssp_id in enumerate(gal_tag_cond[0]):    
        spec_wave_ssp, spec_flux_ssp[idx] = spec_ssp_lookup(age_hacc[ssp_id], 
                                                            metal_hacc[ssp_id], 
                                                            mass_hacc[ssp_id],
                                                            age_fsps_gyr,
                                                            Z_padova_fsps,
    ...
    spec_csp = ssp_to_csp(spec_flux_ssp)
    flux_proxy = total_luminosity(spec_flux_ssp, spec_wave_library) 
    gal_stellar_mass = galaxy_stellar_mass(mass_hacc[gal_tag_cond])
