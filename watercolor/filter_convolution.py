...
# %% auto #0
__all__ = ['ALL_FILTER_DIR', 'FILTERS_LSST', 'FILTERS_SPHEREX', 'FILTERS_COSMOS', 'FILTERS_WISE', 'FILTERS_LS', 'FILTERS_2MASS',
           'FILTERS_F784', 'load_filter_single', 'clip_bandpass_values', 'load_survey_filters', 'load_survey_pickle',
           'photometry_from_spectra']

# %% ../nbs/06_filter_convolve.ipynb #8eb61988
import numpy as np
import glob
import pickle
...
# %% ../nbs/06_filter_convolve.ipynb #09d47b1b
...
# %% ../nbs/06_filter_convolve.ipynb #96229409
def load_filter_single(filtfile:str=None, # Individual filter files 
    ...
    elif ('WISE' in filtfile):
        bandpass_name = filtfile.split('.res')[0].split('/')[-1]
        x = np.loadtxt(filtfile)
        
    elif ('LEGACYSURVEY' in filtfile):
        bandpass_name = filtfile.split('.txt')[0].split('/')[-1]
        x = np.loadtxt(filtfile)
        
    elif ('2MASS' in filtfile):
        bandpass_name = filtfile.split('.txt')[0].split('/')[-1]
        x = np.loadtxt(filtfile)
        
    elif ('F784' in filtfile):
        bandpass_name = filtfile.split('.txt')[0].split('/')[-1]
        x = np.loadtxt(filtfile)
...
# %% ../nbs/06_filter_convolve.ipynb #a5551227
...
# %% ../nbs/06_filter_convolve.ipynb #c265e061
def load_survey_filters(filtdir:str=ALL_FILTER_DIR+'LSST', #Input directory with all filter definitions
    ...
    if ('SPHEREx' in filtdir):
        ...
        for bandidx in bband_idxs:
            ...
            filtfiles = glob.glob(filtdir+'*band'+str(bandidx)+'*.dat')
...
def load_survey_pickle(survey:str='LSST' #Survey
    ...
    if (survey=='LSST'):
        ...
        FILTER_NAME = FILTERS_WISE
    elif (survey=='LEGACYSURVEY'):
        FILTER_NAME = FILTERS_LS
    elif (survey=='2MASS'):
        FILTER_NAME = FILTERS_2MASS
    elif (survey=='F784'):
        FILTER_NAME = FILTERS_F784
...
# %% ../nbs/06_filter_convolve.ipynb #26259330
def photometry_from_spectra(central_wavelengths:np.array=None, # Central wavelengths
    ...
    if plot:
        ...
        a[0].set_ylim(0, 1.2*np.max(flux))
        # a[0].set_yscale('log')
